...
PKG_NAME="keyboard-gpui"
PKG_VERSION="main"
PKG_LICENSE="GPL-3.0-or-later"
PKG_SITE="https://github.com/weihuoya/rocknix-keyboard"
...
PKG_LONGDESC="A GPUI-based on-screen keyboard for ROCKNIX"
PKG_TOOLCHAIN="manual"

# Allow a local checkout to be used instead of fetching from git.
# Set ROCKNIX_KEYBOARD_SOURCE_DIR in the environment to the path containing the source.
if [ -n "${ROCKNIX_KEYBOARD_SOURCE_DIR}" ]; then
  PKG_URL=""
  GET_HANDLER_SUPPORT=""
else
...
unpack() {
  if [ -n "${ROCKNIX_KEYBOARD_SOURCE_DIR}" ]; then
    mkdir -p "${PKG_BUILD}"
    cp -a "${ROCKNIX_KEYBOARD_SOURCE_DIR}/." "${PKG_BUILD}/"
  fi
}
...
makeinstall_target() {
  mkdir -p ${INSTALL}/usr/bin
    cp ${PKG_BUILD}/.${TARGET_NAME}/target/${TARGET_NAME}/release/rocknix-keyboard ${INSTALL}/usr/bin/
}

post_install() {
  enable_service keyboard-gpui.service
}
